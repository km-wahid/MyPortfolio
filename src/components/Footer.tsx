...
const Footer: React.FC<FooterProps> = ({ content, socials }) => {
  ...
  return (
    <footer className="relative py-10 overflow-hidden portfolio-footer-shell">
      ...
      <div
        className="w-full px-4 sm:px-6 lg:px-8 rounded-3xl py-8 portfolio-footer-card"
      ...
      >
          ...
          <div className="flex items-center gap-3">
            {socials.map((s) => (
              <motion.a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-lg flex items-center justify-center text-gray-500 hover:text-neon-blue transition-all duration-300 portfolio-footer-social"
        ...
        {/* Bottom line */}
        <div className="mt-8 pt-6 flex flex-col sm:flex-row justify-between items-center gap-2 portfolio-footer-bottomline"
